<template>
  <section class="p-4 rounded-lg md:ml-64 h-auto pt-20 min-h-screen">
    <div class="text-2xl font-medium tracking-tight text-gray-900 dark:text-white">Projects</div>
    ...
    <div>
      ...
      <template v-if="initialFetch && tableData.length == 0">
        <NoResults label="There Are No Project Please Create One By Clicking On Add Project" />
      </template>
      <template v-else>
        <Table
          :tableData="tableData"
...
export default {
  ...
  methods: {
    ...
    async removeProject(data) {
      const resp = await deleteProject(data.$id, data.status)
      if (resp.success) {
        this.getData(this.pageNo)
      }
    },
